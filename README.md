...
question the builder opens with, before anything else. Three templates
ship today:
...
built yet (The Defense, The Mission, and a longer second-wave list) and
...
- **`game/config.js` / `gallery/config.js` / `flight/config.js`** —
  ...
  `examples/gallery-sample.config.js` (Gallery), and
  `examples/flight-sample.config.js` (Flight) for the real schema
  references instead.
- **`game/skeletons.js`** — the four Hangout **story skeletons**
  (settings): THE DINNER PARTY, THE ROAD TRIP, THE OFFICE PARTY, THE
  WEDDING WEEKEND. Hangout-only — the Gallery and the Flight have no
  scene skeleton of their own. See "Story skeletons" below.
...
- **`game/cfgcodec.js`** — the shared CONFIG codec: compression, the
  whitelist schema (now three templates' worth), the neutral default
...
(Flight, fully cast). Visiting a bare template page directly (`/game/`,
`/intro/`, `/gallery/`, `/flight/`) with no `#cfg=` link redirects to
`/build/` — there's no file config to play there.
...
cinematic; the Gallery's/Flight's own in-page title screen). PRESS
...
treat these as dev tools, not user-facing links. The Gallery and the
Flight have no `?start=` shortcuts (single-screen games, nothing to
skip to).
...
`gallery/engine.js`; `flight/engine.js`) is the *entire* game for that
...
**THE GALLERY / THE FLIGHT — 2 files** (a single page, no separate
intro):

```
games/<slug>/
  config.js    that order's CONFIG -- see examples/gallery-sample.config.js / examples/flight-sample.config.js
  index.html   config.js -> ../../game/roster.js -> ../../game/cfgcodec.js -> ../../shared/framework.js -> ../../gallery/engine.js (or ../../flight/engine.js)
```

`games/gallery-sample/` and `games/flight-sample/` are exactly this,
checked in as real examples — `tools/generate.js` deploys either shape
automatically depending on the order's `template` answer (see "The
generator" below).
...
two for Gallery/Flight), and prints a shareable link. See
...
`/flight/` — this repo's own top-level pages, or in principle any page
...
- **Default + merge**: the decoded fragment deep-merges onto
  ...
  `'gallery'`/`'flight'` each dispatch to their own smaller, separate
  base object) — so a fragment can supply just a few fields (a host
...
- **Whitelist validation**: the decoded object is sanitized against an
  ...
  `'flight'`, append-only) — an unrecognized value sanitizes away
...
- The Hangout's intro↔game page transitions forward `location.hash` so
  the same fragment survives navigating between them (the Gallery and
  the Flight are single pages, so this doesn't apply to them).
...
`SPEC-flight.md` for the Gallery's/Flight's own). Short version, true
across all three templates: every game has a required **HOST** (the
...
Hangout-only (the Gallery and the Flight have no scene skeleton). Every
...
candidates — The Defense, The Mission, and further out) is a bigger
lift than a fifth skeleton — it's a new GAME, not a new room — but the
checklist is fixed now, proven twice (The Gallery, then The Flight):
...
1. **Spec it.** Write `SPEC-<name>.md` — the game, the cast mapping
   ...
   on `SPEC-gallery.md`/`SPEC-flight.md`, which both exist precisely so
   the next template doesn't have to invent this shape from scratch.
2. **Build the engine on `shared/framework.js`.** A single page, no
   ...
   `let CONFIG = null` stub). Mirror `gallery/engine.js`'s / `flight/
   engine.js`'s conventions: `ENGINE_ROOT` from
...
3. **Codec.** In `game/cfgcodec.js`: append the new key to
   ...
   `cfgBuildGalleryDefaultConfig`/`cfgBuildFlightDefaultConfig` (its own
...
4. **Wizard.** In `build/index.html`: a `JOKE_DEFS` card (joke stays
   ...
   wants boss lines (same mechanism as `galleryField`/`flightField`); a
...
5. **`tools/generate.js`.** An `is<Name>` branch in `buildOverrides`
   (required content fields validated the same way `gallery`'s/
   `flight`'s already are); `engineRoot = '../../'` (single page,
   2-deep, same as gallery/flight); the two-file shell deploy via
...
6. **Verify driver.** `tools/verify-<name>.js`, gallery/flight-driver
...
9. **Docs.** `README.md` (this section's own list — add the new
   template everywhere the Gallery/Flight now appear), `tools/README.md`'s
...
Shooting Gallery pack) and `assets/flight/CREDITS.txt` (Kenney's Tappy
Plane pack) alongside `intro/assets/CREDITS.txt` (the Hangout's roster
tile sheets, shared by every template).
